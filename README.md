...
Note: The request pipeline in [hapi](http://hapijs.com/) only allows incoming request methods to be changed at the 'onRequest' extension point of the [request lifecycle](http://hapijs.com/api#request-lifecycle) via `request.setMethod`. At this point however, the request stream has not been processed and so 'payload'/form data is not yet available. This means that in addition to the hidden form field approach taken by Express and Rails, we'll need to include a query string 'signal' to trigger the method change, and then later in the pipeline verify the method against the hidden form field value. The extra verification is needed to protect against malicious changes of querystring values, for example a 'get' request that's been changed to a 'delete' request (and assumes you have form protection via TLS/CSRF etc.).
